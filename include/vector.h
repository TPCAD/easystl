...
template <typename Tp, typename Alloc> struct vector_base {
    ...
    struct vector_impl_data {
        ...
        vector_impl_data(vector_impl_data &&x) noexcept
            : M_start(x.M_start), M_finish(x.M_finish),
        ...
        void M_copy_data(vector_impl_data const &x) noexcept {
            M_start = x.M_start;
            M_finish = x.M_finish;
            M_end_of_storage = x.M_end_of_storage;
        }

        void M_swap_data(vector_impl_data &x) noexcept {
            vector_impl_data tmp;
    ...
    // vector 的重绑定分配器，包含 vector 的数据成员
    struct vector_impl : public Tp_alloc_type, public vector_impl_data {
        vector_impl() noexcept(
            std::is_nothrow_default_constructible<Tp_alloc_type>::value)
            : Tp_alloc_type() {}

        vector_impl(Tp_alloc_type const &a) noexcept : Tp_alloc_type(a) {}
        vector_impl(Tp_alloc_type &&a) noexcept
            : Tp_alloc_type(easystl::move(a)) {}

        vector_impl(vector_impl &&x) noexcept
            : Tp_alloc_type(easystl::move(x)),
              vector_impl_data(easystl::move(x)) {}

        vector_impl(Tp_alloc_type &&a, vector_impl &&x) noexcept
            : Tp_alloc_type(easystl::move(a)),
              vector_impl_data(easystl::move(x)) {}
    };
  ...
  public:
    ...
    // 获取重绑定分配器
    Tp_alloc_type &M_get_Tp_allocator() noexcept { return this->M_impl; }
    ...
    vector_base(const allocator_type &a) noexcept : M_impl(a) {}

    vector_base(std::size_t n) : M_impl() { M_create_storage(n); }

    vector_base(std::size_t n, const allocator_type &a) : M_impl(a) {
        M_create_storage(n);
    }

    vector_base(vector_base &&) = default;

    vector_base(Tp_alloc_type &&a) : M_impl(easystl::move(a)) {}

    vector_base(vector_base &&x, const allocator_type &a) : M_impl(a) {
        if (x.get_allocator() == a) {
            this->M_impl.M_swap_data(x.M_impl);
        } else {
            std::size_t n = x.M_impl.M_finish - x.M_impl.M_start;
            M_create_storage(n);
        }
    }

    vector_base(const allocator_type &a, vector_base &&x)
        : M_impl(Tp_alloc_type(a), easystl::move(x.M_impl)) {}

    ~vector_base() noexcept {
        M_deallocate(M_impl.M_start, M_impl.M_end_of_storage - M_impl.M_start);
    }
  ...
  public:
    ...
    pointer M_allocate(std::size_t n) {
        ...
        return n != 0 ? Tr::allocate(M_impl, n) : pointer();
    }

    void M_deallocate(pointer p, std::size_t n) {
        using Tr = easystl_cxx::alloc_traits<Tp_alloc_type>;
        if (p) {
            Tr::deallocate(M_impl, p, n);
        }
    }
...
//     template <class Iter,
//               typename
//               std::enable_if<easystl::is_input_iterator<Iter>::value,
//                                       int>::type = 0>
...
//     // modifier
//     // assign
//     void assign(size_type n, const value_type &value) { fill_assign(n,
//     value); }
//
//     template <class Iter,
//               typename
...
//         data_allocator::construct(address_of(*new_end),
//         forward<Args>(args)...);
//         ++new_end;
...
// template <class T>
// typename vector<T>::iterator vector<T>::fill_insert(iterator pos, size_type
// n,
//                                                     const value_type &value)
//                                                     {
//     if (n == 0) {
